...
from typing import Any, Callable, TypeVar, Generic, Optional, Iterable
...
def load_json(path: Path, default_factory: Callable[[], _VT] = dict) -> _VT:
    if not path.exists():
...
class Json(defaultdict, Generic[_KT, _VT]):
    def __init__(
        self,
        path: Path | str,
    ...
    def update(self, __m: Optional[
            ...
            | Iterable[tuple[_KT, _VT]]] = None, **kwargs: _VT) -> None:
        if __m is None:
            super().update(**kwargs)
        else:
